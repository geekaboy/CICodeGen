...
foreach ($input_list as $key => $input) {
    $insertListCode .= '\''.$input['column_name'].'\''.'=>'.'$post[\''.$input['column_name'].'\'],
            ';
    $validListCode .= '$frm->set_rules('.'\''.$input['column_name'].'\', \''.$input['label'].'\' '.', \'trim|required\');
    ';
    $validmessageCode .='$message .= form_error(\''.$input['column_name'].'\');
        ';
}
